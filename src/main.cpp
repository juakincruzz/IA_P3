...
void mostrarAyuda() {
    ...
    std::cout << "  -d <int>    Profundidad de búsqueda de la IA (por defecto: 5)" << std::endl;
    std::cout << "  -t <double> Tiempo límite para la IA en segundos (por defecto: 5.0)" << std::endl;
...
std::shared_ptr<Agente> crearAgente(const std::string& tipo, int id, int profundidad, double tiempoMax, int h = 1) {
    ...
    else if (tipo == "status") return std::make_shared<AgenteEstudiante>(id, profundidad, tiempoMax, h, AgenteEstudiante::ModoJuego::STATUS);
